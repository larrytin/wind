...
public class GaeChannel extends ChannelDemuxer {
  ...
  public String getSessionId() {
    return nativeGetVars().getString(Constants.Params.SESSION_ID);
  }
  ...
  private native void connectNative(String token) /*-{
    ...
    var socket = this.@com.goodow.wind.channel.impl.GaeChannel::socket;
    if (socket != null) {
      socket.close();
    }
    ...
    socket = channel.open();
    this.@com.goodow.wind.channel.impl.GaeChannel::socket = socket;
  
    socket.onopen = $entry(function() {
      me.
        @com.goodow.wind.channel.impl.GaeChannel::onOpened()
        ();
    });
    socket.onmessage = $entry(function(msg) {
      me.
        @com.goodow.wind.channel.impl.GaeChannel::publishMessage(Ljava/lang/String;)
        (msg.data);
    });
    socket.onerror = $entry(function(err) {
      me.
        @com.goodow.wind.channel.impl.GaeChannel::onError(ILjava/lang/String;)
        (err.code, err.description);
    });
    socket.onclose = $entry(function() {
      me.
        @com.goodow.wind.channel.impl.GaeChannel::close()
        ();
    });
  }-*/;
  // @formatter:on
