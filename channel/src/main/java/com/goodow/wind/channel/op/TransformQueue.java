 ...
 */
public class TransformQueue<O extends Op<?>> {
  public interface Transformer<M> {
    ArrayOf<M> compose(ArrayOf<M> ops);

    Pair<M, M> transform(M serverOp, M clientOp);
  }

  private final Model<O, ?> model;
  ...
  public TransformQueue(Model<O, ?> model) {
    this.model = model;
  }
  ...
  public void clientOp(O clientOp) {
    ...
    if (!serverOps.isEmpty()) {
      ...
      cOps.push(clientOp);
      Pair<ArrayOf<O>, ArrayOf<O>> pair = model.transform(serverOps, cOps);
  ...
  public ArrayOf<O> pushQueuedOpsToUnacked() {
    ...
    unackedClientOps = model.compose(queuedClientOps);
  ...
  public void serverOp(int resultingRevision, O serverOp) {
    ...
    if (!unackedClientOps.isEmpty()) {
      ...
      sOps.push(serverOp);
      Pair<ArrayOf<O>, ArrayOf<O>> pair = model.transform(sOps, unackedClientOps);
      sOps = pair.first;
    ...
    if (!queuedClientOps.isEmpty() && newClientOpSinceTransform) {
      queuedClientOps = model.compose(queuedClientOps);
    ...
    if (!queuedClientOps.isEmpty()) {
      ...
      Pair<ArrayOf<O>, ArrayOf<O>> pair = model.transform(sOps, queuedClientOps);
